...
CVector<uint8_t> GenTestFrame ( const int iCnt, const int iID, const CVector<uint8_t>& vecbyBody )
{
...
QByteArray ToByteArray ( const CVector<uint8_t>& vecbyData )
{
    return QByteArray ( reinterpret_cast<const char*> ( vecbyData.data() ), vecbyData.Size() );
}

CVector<uint8_t> FromByteArray ( const QByteArray& baData )
{
...
bool ParseFrame ( const CVector<uint8_t>& vecbyFrame, CVector<uint8_t>& vecbyMesBodyData, int& iRecCounter, int& iRecID )
{
...
void ConnectProtocols ( CProtocol& From, CProtocol& To )
{
